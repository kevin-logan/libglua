...
/**
 * Class abstracting the concept of a position on the glua stack. An RAII object
 * where the value on the stack will be popped when it goes out of scope.
 *
 * Unfortunately this creates a complexity where StackPositions must never be
 * used in a way where they destruct in an order different than their
 * construction order, so this class is only intended to be used as l-values on
 * the stack, rather than using something like unique/shared_ptrs to extend or
 * modify their lifetimes.
 */
class StackPosition {
public:
  /**
...
// .tcc implementation file is included by GluaBase.h instead to avoid circular
// dependency