...
#define REGISTER_TO_GLUA(glua, functor) glua.RegisterCallable(#functor, (glua).CreateGluaCallable(functor))

#define REGISTER_CLASS_TO_GLUA(glua, ClassType, ...) glua.RegisterClassMultiString<ClassType>(#__VA_ARGS__, __VA_ARGS__)

namespace kdk::glua
{
/**
 * Base for a Glua instance that can serve as an argument stack for a DeferredArgumentCaller,
 * while providing the shared logic for type deduction used for the various language binding
 * implementation
 *
 */
class GluaBase
...
public:
    ...
    /**
     * @brief pulls an argument off the stack for the given DeferredArgumentCallable, which must
     * be a GluaCallable
     *
    ...
    static auto deferred_argument_get(const ICallable* callable, size_t index) -> Type;
    /**
     * @brief pushes an argument onto the stack, the argument being the return value from
     * a DeferredArgumentCallable's call, where the callable must be a GluaCallable
     *
     ...
     */
    template<typename Type>
    static auto deferred_argument_push(const ICallable* callable, Type&& value) -> void;
    /*************************************************************/

    /** GluaBase public interface, implemented by language specific derivations **/

    /**
     * @brief Resets the environment of this instance, effectively removing any globals
     * or changes that have occurred from running scripts. Registered functions and
     * methods are NOT affected, and will still be callable. Effectively preps this
     * instance to run a script from a fresh state
     *
     * @param sandboxed true if the new environment should be a sandbox environment
     *                  that prevents some security issues like writing to files, etc
     */
    virtual auto ResetEnvironment(bool sandboxed = true) -> void = 0;
    /**
     * @brief registers a callable so that it is available to be called from the scripting
     * environment using the given name
     *
     * @param name the name the callable should be called with from the scripting environment
     * @param callable the callable to execute when called from the scripting environment
     */
    virtual auto RegisterCallable(const std::string& name, Callable callable) -> void = 0;
    /**
    ...
    /**
     * @brief Runs a file, by reading the data in from the file and calling RunScript
     *
     ...
     * as a value of the requested type
     *
     ...
     * is capable of retrieving arguments from the Glua stack, and putting return values
     * back on the glua stack
     *
    ...
    /**
     * @brief Creates a callable object as a GluaCallable from the given function pointer, so it
     * is capable of retrieving arguments from the Glua stack, and putting return values
     * back on the glua stack
     *
    ...
    /**
     * @brief Creates a callable object as a GluaCallable from the given const method pointer, so it
     * is capable of retrieving arguments from the Glua stack, and putting return values
     * back on the glua stack
     *
    ...
    auto CreateGluaCallable(ReturnType (ClassType::*callable)(Params...) const) -> Callable;

    /**
     * @brief Creates a callable object as a GluaCallable from the given method pointer, so it
     * is capable of retrieving arguments from the Glua stack, and putting return values
     * back on the glua stack
     *
     * @tparam ClassType the type of the Class this is a method of
     ...
     */
    template<typename ClassType, typename ReturnType, typename... Params>
    auto CreateGluaCallable(ReturnType (ClassType::*callable)(Params...)) -> Callable;

    /**
     * @brief Registers a class from a multi string. This string is generally generated from
     * REGISTER_CLASS_TO_GLUA, and it's not recommended to call this method directly
     *
     ...
     * @param method_names a comma separated and fully qualified method pointers, e.g.:
     *                     "&ClassName::Foo, &ClassName::Bar"
     * @param methods the method pointers to bind to the comma separated names in method_names
     */
    ...
    /**
     * @brief Registers a class to glua with the given names and callables for methods to expose
     * to the scripting environment for this type. This is generally called from RegsiterClassMulitString
     * and it's notrecommended to call this method directly
     *
     * @tparam ClassType the class type to register to this instance of glua
     * @param method_names the names of the methods to register, must be same length as the methods vector
     *                     NOTE: the method names should still be fully qualified,.e.g "&ClassName::Foo" as
     *                     this method will deduce the class name to use to register from the fully qualified
     *                     names
     * @param methods the callables of the methods to register, must be same length as the method_names vector
     */
    ...
    /**
     * @brief Calls a function in the scripting environment with the given name using the
     * given parameters
     *
...
protected:
    ...
    virtual auto pushUserType(const std::string& unique_type_name, std::unique_ptr<IManagedTypeStorage> user_storage)
        -> void                                                                                                  = 0;
    ...
    virtual auto getArrayValue(size_t index_into_array, int stack_index_of_array) const -> void                  = 0;
    ...
    virtual auto registerClassImpl(
        ...
        std::unordered_map<std::string, std::unique_ptr<ICallable>> method_callables) -> void = 0;
    virtual auto registerMethodImpl(const std::string& class_name, const std::string& method_name, Callable method)
        -> void                                                          = 0;
...
private:
    template<typename Type>
    auto push(const std::vector<Type>& value) -> void;
    template<typename Type>
    auto push(const std::unordered_map<std::string, Type>& value) -> void;
    template<typename Type>
    auto push(const std::optional<Type>& value) -> void;

    template<typename Type>
    auto pushRegisteredType(Type&& custom_type) -> void;

    template<typename Type>
    auto getArray(int stack_index) -> std::vector<Type>;
    template<typename Type>
