...
namespace kdk::glua {
enum class ManagedTypeStorageType { RAW_PTR, SHARED_PTR, STACK_ALLOCATED };
...
template <typename T> class ManagedTypeStorage : public IManagedTypeStorage {
public:
...
template <typename T> class ManagedTypeRawPtr : public ManagedTypeStorage<T> {
public:
  ...
  ~ManagedTypeRawPtr() override =
      default; // we don't own the object, so we don't delete it
