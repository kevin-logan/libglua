    ...
    : m_lua(luaL_newstate()), m_output_stream(output_stream), m_current_array_index(0)
{
    ...
    glua_set_sub_environment_to_stack(m_lua.get(), "coroutine", sandbox_coroutine_environment);
    glua_set_sub_environment_to_stack(m_lua.get(), "io", sandbox_io_environment);
    glua_set_sub_environment_to_stack(m_lua.get(), "string", sandbox_string_environment);
    glua_set_sub_environment_to_stack(m_lua.get(), "table", sandbox_table_environment);
    glua_set_sub_environment_to_stack(m_lua.get(), "math", sandbox_math_environment);
    glua_set_sub_environment_to_stack(m_lua.get(), "os", sandbox_os_environment);

    lua_pushlightuserdata(m_lua.get(), &m_output_stream);
...
auto GluaLua::ResetEnvironment(bool sandboxed) -> void
{
    ...
    lua_pushvalue(m_lua.get(), -4);          // push (__libglua__sandbox__ or _G), env -> env -> __index -> sandbox
...
{
    auto code = luaL_loadbuffer(m_lua.get(), script_data.data(), script_data.size(), "libglua");

    if (code == 0)
    {
        lua_getglobal(m_lua.get(), "__libglua__env__");
        lua_setfenv(m_lua.get(), -2);
        code = lua_pcall(m_lua.get(), 0, 0, 0);
    ...
    {
        throw exceptions::LuaException(std::string{"Failed to load script"}.append(lua_tostring(m_lua.get(), -1)));
    }
}
...
auto GluaLua::pushUserType(const std::string& unique_type_name, std::unique_ptr<IManagedTypeStorage> user_storage) -> void
{
    auto** managed_type_ptr =
        static_cast<IManagedTypeStorage**>(lua_newuserdata(m_lua.get(), sizeof(IManagedTypeStorage*)));
...
{
    auto** managed_type_ptr =
        static_cast<IManagedTypeStorage**>(luaL_checkudata(m_lua.get(), stack_index, unique_type_name.data()));
...
auto GluaLua::isUserType(const std::string& unique_type_name, int stack_index) const -> bool
{
...
auto GluaLua::setGlobalFromStack(const std::string& name, int stack_index) -> void
{
    ...
    lua_getglobal(m_lua.get(), "__libglua__env__");
    lua_pushlstring(m_lua.get(), name.data(), name.size());
    lua_pushvalue(m_lua.get(), absolute_value_index); // get value from original stack back into position
...
{
    lua_pop(m_lua.get(), count);
}
auto GluaLua::getStackTop() -> int
{
...
auto GluaLua::callScriptFunctionImpl(const std::string& function_name, size_t arg_count) -> void
{
    ...
    {
        throw exceptions::LuaException(
            "Failed to call lua script function [" + function_name + "]: " + lua_tostring(m_lua.get(), -1));
    }
}
...
auto GluaLua::setValueOfGlobalFromTopOfStack(const std::string& global_name) -> void
{
    lua_getglobal(m_lua.get(), "__libglua__env__");
    lua_pushlstring(m_lua.get(), global_name.data(), global_name.size());
    lua_pushvalue(m_lua.get(), -3); // get value from original stack back into position

    lua_settable(m_lua.get(), -3); // now stack has original value, table, poth should be popped
