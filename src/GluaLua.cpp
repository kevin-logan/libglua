...
static auto glua_capture_print(lua_State *lua) -> int {
  ...
  auto &ostream =
      *static_cast<std::ostream *>(lua_touserdata(lua, lua_upvalueindex(1)));
...
static auto
glua_set_sub_environment_to_stack(lua_State *lua, std::string_view parent_table,
...
GluaLua::GluaLua(std::ostream &output_stream, bool start_sandboxed)
    : m_lua(luaL_newstate()), m_output_stream(output_stream),
  ...
  *managed_type_ptr = user_storage.get();
  ...
  if (lua_isnil(m_lua.get(), -1)) {
    ...
    throw std::runtime_error(
        "Pushing class type which has not been registered [null metatable]! " +
        unique_type_name);
  }
...
auto GluaLua::isUserType(const std::string &unique_type_name,
  ...
  return luaL_checkudata(m_lua.get(), stack_index, unique_type_name.data()) !=
         nullptr;
}
...
auto GluaLua::callScriptFunctionImpl(const std::string &function_name,
  ...
  if (!lua_isfunction(m_lua.get(), -1)) {
    throw exceptions::LuaException("Attempted to call lua script function " +
                                   function_name + " which was not a function");
  }
...
auto GluaLua::registerClassImpl(
  ...
  auto pos_pair =
      m_method_registry.emplace(class_name, std::move(method_callables));
...
auto GluaLua::registerMethodImpl(const std::string &class_name,
  ...
  if (pos_pair.second) {
    ...
    throw exceptions::LuaException(
        "Tried to register method with already registered name [" +
        method_name + "]");
  }
...
auto call_callable_from_lua(lua_State *state) -> int {
  auto *callable_ptr =
      static_cast<ICallable *>(lua_touserdata(state, lua_upvalueindex(1)));
...
auto destruct_managed_type(lua_State *state) -> int {
  auto **managed_type_ptr =
      static_cast<IManagedTypeStorage **>(lua_touserdata(state, 1));
