...
GluaLua::GluaLua(std::ostream& output_stream, bool start_sandboxed)
    : m_lua(luaL_newstate()), m_output_stream(output_stream), m_current_array_index(0)
{
    luaL_openlibs(m_lua);
...
auto GluaLua::RegisterCallable(const std::string& name, Callable callable) -> void
{
    ...
    if (insert_pair.second)
    {
        ...
        lua_pushvalue(m_lua, -3); // push closer back on stack
        lua_settable(m_lua, -3);
        lua_pop(m_lua, 1);

        // now the original closure is all that's left on the stack, set to global env too
...
{
    lua_pushinteger(m_lua, static_cast<lua_Integer>(value));
}
...
auto GluaLua::push(double value) -> void
{
...
{
    lua_createtable(m_lua, static_cast<int>(size_hint), 0);
}
auto GluaLua::pushStartMap(size_t size_hint) -> void
{
    lua_createtable(m_lua, 0, static_cast<int>(size_hint));
}
...
auto GluaLua::pushUserType(const std::string& unique_type_name, std::unique_ptr<IManagedTypeStorage> user_storage) -> void
{
    auto** managed_type_ptr = static_cast<IManagedTypeStorage**>(lua_newuserdata(m_lua, sizeof(IManagedTypeStorage*)));

    *managed_type_ptr = user_storage.get();
    user_storage.release();

    // set the metatable for this object
    luaL_getmetatable(m_lua, unique_type_name.data());
    ...
    if (lua_isnil(m_lua, -1))
    {
        ...
        throw std::runtime_error("Pushing class type which has not been registered [null metatable]! " + unique_type_name);
    }
...
{
    return static_cast<bool>(lua_toboolean(m_lua, stack_index));
}
auto GluaLua::getInt8(int stack_index) const -> int8_t
{
    return static_cast<int8_t>(lua_tointeger(m_lua, stack_index));
}
auto GluaLua::getInt16(int stack_index) const -> int16_t
{
    return static_cast<int16_t>(lua_tointeger(m_lua, stack_index));
}
auto GluaLua::getInt32(int stack_index) const -> int32_t
{
    return static_cast<int32_t>(lua_tointeger(m_lua, stack_index));
}
auto GluaLua::getInt64(int stack_index) const -> int64_t
{
    return static_cast<int64_t>(lua_tointeger(m_lua, stack_index));
}
auto GluaLua::getUInt8(int stack_index) const -> uint8_t
{
    return static_cast<uint8_t>(lua_tointeger(m_lua, stack_index));
}
auto GluaLua::getUInt16(int stack_index) const -> uint16_t
{
    return static_cast<uint16_t>(lua_tointeger(m_lua, stack_index));
}
auto GluaLua::getUInt32(int stack_index) const -> uint32_t
{
    return static_cast<uint32_t>(lua_tointeger(m_lua, stack_index));
}
auto GluaLua::getUInt64(int stack_index) const -> uint64_t
{
    return static_cast<uint64_t>(lua_tointeger(m_lua, stack_index));
}
auto GluaLua::getFloat(int stack_index) const -> float
{
    return static_cast<float>(lua_tonumber(m_lua, stack_index));
}
auto GluaLua::getDouble(int stack_index) const -> double
{
    return static_cast<double>(lua_tonumber(m_lua, stack_index));
}
auto GluaLua::getCharPointer(int stack_index) const -> const char*
{
    return lua_tostring(m_lua, stack_index);
}
auto GluaLua::getStringView(int stack_index) const -> std::string_view
{
    size_t      length;
    const auto* c_str = lua_tolstring(m_lua, stack_index, &length);
...
{
    if (lua_istable(m_lua, stack_index))
    {
        return lua_objlen(m_lua, stack_index);
    }

    throw exceptions::LuaException("GetArraySize for non-table value");
}
auto GluaLua::getArrayValue(size_t index_into_array, int stack_index_of_array) const -> void
{
    auto absolute_array_index = absoluteIndex(stack_index_of_array);
    lua_pushinteger(m_lua, static_cast<lua_Integer>(index_into_array));
    lua_gettable(m_lua, absolute_array_index);
}
auto GluaLua::getMapKeys(int stack_index) const -> std::vector<std::string>
{
    ...
    if (lua_istable(m_lua, stack_index))
    {
        auto size = lua_objlen(m_lua, stack_index);
        ...
        lua_pushnil(m_lua);
        while (lua_next(m_lua, absolute_map_index) != 0)
        {
            // key then value were pushed onto stack
            lua_pop(m_lua, 1); // pop off value, we're not looking

            if (lua_isstring(m_lua, -1) != 0)
            {
                size_t      str_len = 0;
                const char* str     = lua_tolstring(m_lua, -1, &str_len);
                result.emplace_back(str, str_len);
            }
            else
            {
                // key isn't string, lua_tolstring will convert it to a string and mess up iteration, create copy
...
auto GluaLua::getMapValue(const std::string& key, int stack_index_of_map) const -> void
{
    auto absolute_map_index = absoluteIndex(stack_index_of_map);
    lua_pushlstring(m_lua, key.data(), key.size());
    lua_gettable(m_lua, absolute_map_index);
}
auto GluaLua::getUserType(const std::string& unique_type_name, int stack_index) const -> IManagedTypeStorage*
{
    auto** managed_type_ptr =
        static_cast<IManagedTypeStorage**>(luaL_checkudata(m_lua, stack_index, unique_type_name.data()));
    ...
    throw exceptions::LuaException("Tried to get type but invalid value at index!");
}
auto GluaLua::isNull(int stack_index) const -> bool
{
    return lua_isnil(m_lua, stack_index) != 0;
}
auto GluaLua::isBool(int stack_index) const -> bool
{
    return lua_isboolean(m_lua, stack_index) != 0;
}
auto GluaLua::isInt8(int stack_index) const -> bool
{
    return lua_isnumber(m_lua, stack_index) != 0;
}
...
{
    return lua_isstring(m_lua, stack_index) != 0;
}
...
{
    return lua_istable(m_lua, stack_index) != 0;
}
auto GluaLua::isMap(int stack_index) const -> bool
{
    return lua_istable(m_lua, stack_index) != 0;
}
auto GluaLua::setGlobalFromStack(const std::string& name, int stack_index) -> void
{
    auto absolute_value_index = absoluteIndex(stack_index);
    lua_getglobal(m_lua, "__libglua__env__");
    lua_pushlstring(m_lua, name.data(), name.size());
    lua_pushvalue(m_lua, absolute_value_index); // get value from original stack back into position

    lua_settable(m_lua, -3);

    lua_pop(m_lua, 1); // env table is still on stack, pop
}
...
auto GluaLua::callScriptFunctionImpl(const std::string& function_name, size_t arg_count) -> void
{
    ...
    // arg_count arguments were already on the stack, but lua requires the function before the arguments
    if (arg_count > 0)
    {
        // need to move the function back arg_count layers on the stack
        lua_insert(m_lua, -1 - static_cast<int>(arg_count));
    }

    lua_getglobal(m_lua, "__libglua__env__");
    lua_setfenv(m_lua, -2);
    if (lua_pcall(m_lua, arg_count, LUA_MULTRET, 0) != 0)
    {
        throw exceptions::LuaException(
...
auto GluaLua::registerMethodImpl(const std::string& class_name, const std::string& method_name, Callable method) -> void
{
    luaL_getmetatable(m_lua, class_name.data());

    auto pos_pair = m_method_registry[class_name].emplace(method_name, std::move(method).AcquireCallable());
...
auto GluaLua::GluaLua::pushValueOfGlobalOntoStack(const std::string& global_name) -> void
{
    lua_getglobal(m_lua, "__libglua__env__");
...
auto GluaLua::setValueOfGlobalFromTopOfStack(const std::string& global_name) -> void
{
    lua_getglobal(m_lua, "__libglua__env__");
    lua_pushlstring(m_lua, global_name.data(), global_name.size());
...
auto GluaLua::absoluteIndex(int index) const -> int
{
    ...
    return lua_gettop(m_lua) + index + 1;
}
